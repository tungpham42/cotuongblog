...
@php
    // Determine if this is the English Portal
...
<div class="space-y-8">
            ...
            <header class="relative bg-white/60 dark:bg-slate-800/70 backdrop-blur-2xl rounded-[2.5rem] shadow-[0_15px_40px_rgba(249,115,22,0.08)] dark:shadow-[0_15px_40px_rgba(0,0,0,0.4)] border border-white/80 dark:border-slate-700/60 overflow-hidden p-8 sm:p-10 lg:p-12 transition-all duration-700 hover:shadow-[0_25px_50px_rgba(249,115,22,0.12)] dark:hover:shadow-[0_25px_50px_rgba(0,0,0,0.5)] group">
                ...
                <div class="relative z-10 flex flex-col sm:flex-row items-center sm:items-start gap-6 sm:gap-8">
                    <div class="shrink-0 group/img relative">
                        ...
                        @else
                            <div class="w-28 h-28 sm:w-32 sm:h-32 rounded-[1.25rem] bg-gradient-to-br from-brand/20 to-brand/5 dark:from-brand/30 dark:to-brand/10 flex items-center justify-center shadow-xl text-brand border-2 border-white dark:border-slate-700/80 relative z-10 backdrop-blur-sm">
                    ...
                    <div class="flex flex-col text-center sm:text-left flex-grow justify-center min-h-[7rem]">
                        <div class="flex flex-col sm:flex-row sm:items-center gap-4 mb-3 sm:mb-4">
                            ...
                            <span class="inline-flex items-center justify-center px-4 py-1.5 rounded-full bg-white/80 dark:bg-slate-800/80 text-brand text-sm font-bold whitespace-nowrap self-center shadow-sm border border-brand/20">
    ...
    <div id="articles" class="flex flex-col gap-5 border-b border-slate-200 dark:border-slate-700 pb-5 mb-6 pt-4">
        <form action="{{ route($isEn ? 'categories.show.en' : 'categories.show', $category->slug) }}" method="GET" class="relative z-30 w-full">
            <div class="flex flex-col sm:flex-row items-center w-full bg-white/80 dark:bg-slate-800/80 backdrop-blur-xl rounded-[2rem] sm:rounded-full shadow-sm border border-slate-200/80 dark:border-slate-700/80 p-1.5 gap-2 transition-all duration-500 focus-within:ring-4 focus-within:ring-brand/10">

                <div class="relative flex items-center w-full flex-grow group/search">
                    <div class="absolute left-4 text-brand/60 group-focus-within/search:text-brand transition-colors">
                    ...
                    <input type="text"
                        ...
                        class="w-full bg-transparent border-none outline-none focus:ring-0 text-slate-800 dark:text-slate-100 placeholder-slate-400 pl-11 pr-14 py-2.5 text-[15px] font-medium transition-all duration-300">

                    <button type="submit" aria-label="Tìm kiếm" class="absolute right-1.5 top-1/2 -translate-y-1/2 w-9 h-9 flex items-center justify-center rounded-full bg-brand/10 hover:bg-brand text-brand hover:text-white dark:bg-brand/20 dark:hover:bg-brand transition-all duration-300">
                ...
                <div x-data="{
                    ...
                    <button type="button" @click="open = !open" class="flex items-center justify-between w-full sm:w-[250px] bg-slate-50 dark:bg-slate-900/50 hover:bg-brand hover:text-white rounded-[1.5rem] sm:rounded-full px-5 py-2.5 text-sm font-bold transition-all duration-300 group/btn">
                        <div class="flex items-center gap-2.5">
                            <div class="p-1 rounded-md bg-slate-200/50 dark:bg-slate-700/50 group-hover/btn:bg-white/20">
                        ...
                        <svg class="w-4 h-4 ml-2 transition-transform duration-300" :class="{'rotate-180': open}" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M19 9l-7 7-7-7"></path></svg>
                    </button>

                    <div x-show="open" x-transition class="absolute right-0 z-50 w-full sm:w-[260px] mt-3 bg-white/95 dark:bg-slate-800/95 backdrop-blur-xl border border-slate-100 dark:border-slate-700/80 rounded-[1.5rem] shadow-2xl p-2" style="display: none;">
                        <div class="space-y-1">
                            <template x-for="(label, value) in options" :key="value">
                                <button type="button" @click="selectOption(value)" class="w-full text-left px-4 py-3 text-[14px] rounded-xl flex items-center justify-between transition-all" :class="{'bg-brand text-white font-bold': selected === value, 'text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700/50': selected !== value}">
    ...
    <section aria-label="Article List">
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 lg:gap-8">
            @forelse ($posts as $post)
                <article class="group flex flex-col bg-white dark:bg-slate-800 rounded-[2rem] shadow-sm border border-brand/5 dark:border-slate-700/80 overflow-hidden hover:shadow-[0_20px_40px_rgba(249,115,22,0.15)] transition-all duration-500 transform hover:-translate-y-2 relative">
                    ...
                    <div class="p-6 sm:p-8 flex flex-col flex-grow relative bg-white dark:bg-slate-800 z-10 rounded-b-[2rem]">
                        <div class="absolute -top-5 left-6 sm:left-8">
                            <span class="inline-flex items-center gap-1.5 bg-gradient-to-r {{ $isEn ? 'from-blue-600 to-brand' : 'from-brand to-orange-400' }} text-white text-[11px] uppercase tracking-wider font-black px-4 py-2.5 rounded-xl shadow-lg transform group-hover:-translate-y-1 transition-transform duration-500">
                        ...
                        <div class="flex items-center justify-between mt-auto pt-5 border-t border-brand/10 dark:border-slate-700/50">
                            <div class="flex items-center gap-3">
                                <div class="w-9 h-9 rounded-full bg-orange-50 dark:bg-brand/20 flex items-center justify-center text-brand font-black text-sm border border-brand/10">
                            ...
                            <a href="{{ route('posts.show', $post->slug) }}" class="flex items-center gap-1.5 text-sm font-black text-brand opacity-0 -translate-x-4 group-hover:opacity-100 group-hover:translate-x-0 transition-all duration-300">
            ...
            @empty
                <div class="col-span-full py-20 text-center bg-white dark:bg-slate-800 rounded-2xl border border-dashed border-slate-300 dark:border-slate-700">
                    ...
                    <a href="{{ route('home') }}" class="inline-flex items-center justify-center px-6 py-2.5 bg-brand text-white font-medium rounded-xl hover:bg-orange-600 transition-colors shadow-sm">
