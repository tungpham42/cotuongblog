                ...
                <div class="shrink-0 group/img relative">
                    ...
                    @if($category->featured_image)
                        <figure class="w-28 h-28 sm:w-32 sm:h-32 rounded-[1.25rem] overflow-hidden shadow-xl border-2 border-white dark:border-slate-700/80 relative z-10 bg-white dark:bg-slate-800">
                            <img src="{{ asset('storage/' . $category->featured_image) }}" alt="Ảnh chuyên mục {{ $category->name }}" class="w-full h-full object-cover transform group-hover/img:scale-110 group-hover/img:rotate-3 transition-all duration-700 ease-out">
