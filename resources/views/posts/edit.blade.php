    ...
    <div class="px-6 py-8 sm:p-10">
        ...
        <form id="post-form" action="{{ route('posts.update', $post) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            <div class="bg-slate-50 dark:bg-slate-800/50 p-6 rounded-xl border border-slate-200 dark:border-slate-700 mt-6">
                ...
                <input type="url" name="video_url" value="{{ old('video_url', $post->video_url) }}" placeholder="https://www.youtube.com/embed/..."
            ...
            @if(auth()->user()->is_admin)
                <div class="bg-slate-50 dark:bg-slate-900 p-6 rounded-xl border border-slate-200 dark:border-slate-700">
                    ...
                    <p class="text-xs text-slate-500 dark:text-slate-400 mb-4 font-normal">Chọn trạng thái hiển thị cho bài viết này.</p>

                    <div x-data="{
                            open: false,
                            selectedValue: '{{ old('is_published', $post->is_published) }}',
                            selectedLabel: '',
                            options: [
                                { value: '1', label: 'Có (Xuất bản công khai)' },
                                { value: '0', label: 'Không (Lưu thành Bản nháp / Ẩn)' }
                            ],
                            init() {
