...
@section('meta_description', \Illuminate\Support\Str::limit(html_entity_decode(strip_tags(\Illuminate\Support\Str::markdown($post->excerpt ?? $post->content)), ENT_QUOTES | ENT_HTML5, 'UTF-8'), 160))
        ...
        <div class="p-6 sm:p-10 lg:p-12">
            ...
            <div class="flex flex-wrap items-center gap-4 mb-6">
                ...
                <div class="flex items-center gap-3">
                    ...
                    {{-- Thời gian đọc --}}
                    <span class="text-slate-500 dark:text-slate-400 text-sm font-semibold flex items-center gap-1.5" title="{{ $isEnglish ? 'Estimated reading time' : 'Thời gian đọc ước tính' }}">
            ...
            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 py-5 border-y border-slate-100 dark:border-slate-700/80 mb-10 bg-slate-50/50 dark:bg-slate-800/50 rounded-2xl px-4 sm:px-6">
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        <div class="text-xs text-slate-400 dark:text-slate-500 font-bold uppercase tracking-wider mb-0.5">{{ $isEnglish ? 'Author' : 'Tác giả' }}</div>
                        <div class="font-bold text-slate-900 dark:text-slate-100 text-base">{{ $post->author->name ?? ($isEnglish ? 'Guest' : 'Khách truy cập') }}</div>
                    </div>
                </div>
                <div class="flex items-center gap-2 text-slate-500 dark:text-slate-400 bg-white dark:bg-slate-900 px-4 py-2 rounded-xl shadow-sm border border-slate-100 dark:border-slate-700 font-semibold text-sm">
                    ...
                    {{ number_format($post->views ?? 0) }} {{ $isEnglish ? 'views' : 'lượt xem' }}
                </div>
            </div>
            ...
            <div id="toc-container" class="mb-10 p-6 sm:p-8 bg-brand/5 dark:bg-brand/10 rounded-[2rem] border border-brand/10 dark:border-brand/20 relative overflow-hidden group">
                ...
                <div class="flex items-center gap-3 mb-5 relative z-10">
                    ...
                    <h3 id="toc-heading" class="text-xl font-black text-slate-900 dark:text-white" style="scroll-margin-top: 6rem;">{{ $isEnglish ? 'Table of Contents' : 'Mục lục' }}</h3>
                </div>
            ...
            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-6 py-6 border-y border-slate-100 dark:border-slate-700/80 mb-12">
                ...
                <div class="flex items-center gap-3 shrink-0">
                    ...
                    {{-- Tumblr --}}
                    <a href="https://www.tumblr.com/widgets/share/tool?canonicalUrl={{ urlencode(url()->current()) }}&title={{ urlencode($post->title) }}" target="_blank" rel="noopener noreferrer" class="w-10 h-10 flex items-center justify-center rounded-full bg-slate-50 dark:bg-slate-900 border border-slate-100 dark:border-slate-700 text-slate-500 hover:bg-[#36465D] hover:text-white hover:border-[#36465D] shadow-sm hover:shadow-[0_4px_15px_rgba(54,70,93,0.3)] transition-all duration-300 hover:-translate-y-1" title="{{ $isEnglish ? 'Share on Tumblr' : 'Chia sẻ qua Tumblr' }}">
                    ...
                    {{-- Discord --}}
                    <a href="https://discord.com/channels/@me" target="_blank" rel="noopener noreferrer" class="w-10 h-10 flex items-center justify-center rounded-full bg-slate-50 dark:bg-slate-900 border border-slate-100 dark:border-slate-700 text-slate-500 hover:bg-[#5865F2] hover:text-white hover:border-[#5865F2] shadow-sm hover:shadow-[0_4px_15px_rgba(88,101,242,0.3)] transition-all duration-300 hover:-translate-y-1" title="{{ $isEnglish ? 'Share on Discord' : 'Chia sẻ qua Discord' }}">
                        <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 24 24"><path d="M20.317 4.3698a19.7913 19.7913 0 00-4.8851-1.5152.0741.0741 0 00-.0785.0371c-.211.3753-.4447.8648-.6083 1.2495-2.2247-.3326-4.4463-.3326-6.6362 0-.1647-.3939-.4062-.8742-.6177-1.2495a.077.077 0 00-.0785-.037 19.7363 19.7363 0 00-4.8852 1.515.0699.0699 0 00-.0321.0277C.5334 9.0458-.319 13.5799.0992 18.057a.0824.0824 0 00.0312.0561c2.0528 1.5076 4.0413 2.4228 5.9929 3.0294a.0777.0777 0 00.0842-.0276c.4616-.6304.8731-1.2952 1.226-1.9942a.076.076 0 00-.0416-.1057c-.6528-.2476-1.2743-.5495-1.8722-.8923a.077.077 0 01-.0076-.1277c.1258-.0943.2517-.1923.3718-.2914a.0743.0743 0 01.0776-.0105c3.9278 1.7933 8.18 1.7933 12.0614 0a.0739.0739 0 01.0785.0095c.1202.099.246.1981.3728.2924a.077.077 0 01-.0066.1276 12.2986 12.2986 0 01-1.873.8914.0766.0766 0 00-.0407.1067c.3604.698.7719 1.3628 1.225 1.9932a.076.076 0 00.0842.0286c1.961-.6067 3.9495-1.5219 6.0023-3.0294a.077.077 0 00.0313-.0552c.5004-5.177-.8382-9.6739-3.5485-13.6604a.061.061 0 00-.0312-.0286zM8.02 15.3312c-1.1825 0-2.1569-1.0857-2.1569-2.419 0-1.3332.9555-2.4189 2.157-2.4189 1.2108 0 2.1757 1.0952 2.1568 2.419 0 1.3332-.9555 2.4189-2.1569 2.4189zm7.9748 0c-1.1825 0-2.1569-1.0857-2.1569-2.419 0-1.3332.9554-2.4189 2.1569-2.4189 1.2108 0 2.1757 1.0952 2.1568 2.419 0 1.3332-.946 2.4189-2.1568 2.4189z"/></svg>
                    </a>
            ...
            <div class="mt-8">
                <div class="mb-8">
                    <h3 class="text-2xl font-black text-slate-900 dark:text-white tracking-tight flex items-center gap-3">
                        ...
                        {{ $isEnglish ? 'Comments' : 'Bình luận' }} ({{ $post->comments->count() }})
                    </h3>
                </div>
                ...
                @else
                    <div class="bg-slate-50 dark:bg-slate-900/50 p-8 rounded-[1.5rem] border border-dashed border-slate-200 dark:border-slate-700 text-center mb-12">
                        ...
                        <p class="text-slate-600 dark:text-slate-300 font-medium">{{ $isEnglish ? 'You need to' : 'Bạn cần' }} <a href="{{ route('login') }}" class="text-brand font-bold hover:underline">{{ $isEnglish ? 'log in' : 'đăng nhập' }}</a> {{ $isEnglish ? 'to join the discussion.' : 'để tham gia thảo luận.' }}</p>
                    </div>
                @endauth
                ...
                <div class="space-y-6">
                        ...
                        <div class="bg-white dark:bg-slate-800 rounded-[1.5rem] p-6 border border-slate-100 dark:border-slate-700 shadow-sm hover:shadow-md transition-shadow">
                            <div class="flex items-center gap-4 mb-4">
                                ...
                                <div>
                                    ...
                                    <p class="text-xs text-slate-500 dark:text-slate-400 font-medium mt-0.5">{{ $comment->created_at->locale($isEnglish ? 'en' : 'vi')->diffForHumans() }}</p>
                                </div>
                                ...
                                @if(auth()->check() && auth()->user()->is_admin)
                                    <div class="ml-auto">
                                        <form action="{{ route('comments.destroy', $comment) }}" method="POST" class="delete-comment-form">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" class="text-slate-400 hover:text-red-500 bg-slate-50 hover:bg-red-50 dark:bg-slate-700 dark:hover:bg-red-500/20 p-2.5 rounded-xl transition-colors" title="{{ $isEnglish ? 'Delete comment' : 'Xóa bình luận' }}">
                    ...
                    @empty
                        <div class="text-center py-10">
                            <p class="text-slate-400 dark:text-slate-500 font-medium italic">{{ $isEnglish ? 'No comments yet. Be the first to share your thoughts!' : 'Chưa có bình luận nào. Hãy chia sẻ suy nghĩ đầu tiên của bạn!' }}</p>
                        </div>
                    @endforelse
                </div>
            </div>
    ...
    @if ($relatedPosts->count() > 0)
        <section class="mt-16">
            <h3 class="text-2xl font-black text-slate-900 dark:text-white mb-8 flex items-center gap-3 relative z-10">
                ...
                {{ $isEnglish ? 'Related Posts' : 'Bài viết liên quan' }}
            </h3>
                        ...
                        <div class="p-6 flex flex-col flex-grow relative bg-white dark:bg-slate-800 z-10 rounded-b-[2rem]">
                            <div class="absolute -top-4 left-6">
                                <span class="inline-flex items-center gap-1 bg-brand text-white text-[10px] uppercase tracking-wider font-extrabold px-3 py-1.5 rounded-lg shadow-md shadow-brand/30 transform group-hover:-translate-y-1 transition-transform duration-500">
                                    {{ $rPost->created_at->locale($isEnglish ? 'en' : 'vi')->diffForHumans() }}
                                </span>
                            </div>
...
@if(!empty($toc))
    <div x-data="{ showTocBtn: false }"
        ...
        <button @click="document.getElementById('toc-heading').scrollIntoView({ behavior: 'smooth' })"
            ...
            <span class="text-sm tracking-wide">{{ $isEnglish ? 'Table of Contents' : 'Mục lục' }}</span>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        deleteForms.forEach(form => {
            form.addEventListener('submit', function(e) {
                ...
                Swal.fire({
                    title: '{{ $isEnglish ? "Delete this comment?" : "Xóa bình luận này?" }}',
                    text: '{{ $isEnglish ? "This action cannot be undone!" : "Hành động này không thể hoàn tác!" }}',
                    icon: 'warning',
                    showCancelButton: true,
                    confirmButtonColor: '#ef4444',
                    cancelButtonColor: isDark ? '#475569' : '#94a3b8',
                    confirmButtonText: '{{ $isEnglish ? "Yes, delete it!" : "Đồng ý, xóa!" }}',
                    cancelButtonText: '{{ $isEnglish ? "Cancel" : "Hủy" }}',
